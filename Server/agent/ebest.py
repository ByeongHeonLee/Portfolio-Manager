import configparser   # *.ini 파일 Parser
...
class XASession:
    """
    사용자의 연결 상태 정보를 관리하기 위한 Object이다.
    
    [Methods]
    OnLogin(self, code, msg)
    OnDisconnect(self)
    """
    ...
    def OnLogin(self, code, msg):
        ...
        [Parameters]
        code : 로그인 상태 코드 ('0000' : 로그인 성공) (str)
        msg  : 로그인 상태 메시지 (str)
...
class XAQuery:
    """
    조회 TR을 처리하는 Object이다.
    하나의 XAQuery Obejct는 하나의 지정된 TR만을 처리한다.

    [Methods]
    OnReceiveData(self, code)
    OnReceiveMessage(self, error, code, message)
    """

    RES_PATH ="C:\\eBEST\\xingAPI\\Res\\"
    tr_run_state = 0  # 현재 TR의 실행여부 (0 : Idle / 1 : Running)

    def OnReceiveData(self, code):
        """
        요청한 API에 대해 데이터를 수신했을 때
        코드를 출력하고 tr_run_state를 1(Running)로 설정한다.

        [Parameters]
        code : 데이터 수신 상태 코드 (str)
    ...
    def OnReceiveMessage(self, error, code, message):
        """
        요청한 API에 대해 메시지를 수신했을 때
        전달받은 에러코드와 메시지를 출력한다.
...
class EBest:
    """
    로그인/로그아웃을 처리하는 Object

    [Methods]
    __init__(self, mode=None)
    login(self)
    logout(self)
    _execute_query(self, res, in_block_name, out_block_name, *out_fields, **set_fields)
    get_tick_size(self, price)
    get_current_call_price_by_code(self, code=None)
    get_stock_price_by_code(self, code=None, cnt="1")
    get_code_list(self, market=None)
    get_credit_trend_by_code(self, code=None, date=None)
    get_agent_trend_by_code(self, code=None ,fromdt=None, todt=None)
    get_short_trend_by_code(self, code=None, sdate=None, edate=None)
    get_theme_by_code(self, code)
    get_theme_list(self)
    get_category_list(self)
    get_price_by_category(self, upcode=None)
    get_price_by_theme(self, tmcode=None)
    get_event_by_code(self, code=None, date=None)
    get_trade_history(self, count=None)
    get_account_info(self)
    get_account_stock_info(self)
    order_check(self, order_no=None)
    order_check2(self, date, code, order_no=None)
    order_stock(self, code, qty, price, bns_type, order_type="00")
    order_cancel(self, order_no, code, qty)
    get_price_n_min_by_code(self, date, code, tick=None)
    """
    QUERY_LIMIT_10MIN = 200   # 200rep
    LIMIT_SECONDS = 600       # 600s = 10min
    # TR 조회는 10분에 200회 이내로 제한되어 있다.
    # 이 제한을 위반할 경우, 프로그램을 Blocked된다.
    # Blocked된 프로그램은 종료한 다음 다시 실행해야 TR을 수행할 수 있다.
    ...
    def __init__(self, mode=None):
        ...
        [Parameters]
        mode : 매매서버 구분, 모의서버는 "DEMO", 실서버는 "PROD"로 구분 (str) (default=None)
    ...
    def login(self):
        """
        XASession Object를 이용해 서버에 연결하고 로그인을 시도한다.
        ...
        while XASession.login_state == 0: # 로그인 되기까지 윈도우에서 발생하는 메시지를 출력
            pythoncom.PumpWaitingMessages()

    def logout(self):
        """
        XASession Object를 이용해 서버에 연결을 종료하고 로그아웃한다.
    ...
    def get_current_call_price_by_code(self, code=None):
        ...
        [Parameters]
        code : 종목코드 (str) (default=None)

        [Returns]
        result : 해당 종목의 최근 가격 정보들이 저장된 딕셔너리의 리스트 (list)
        """
    ...
    def get_stock_price_by_code(self, code=None, cnt="1"):
        """
        TR : t1305
        현재 날짜를 기준으로 cnt 만큼 전일의 데이터를 가져온다.

        [Parameters]
        code : 종목코드 (str) (default=None)
        cnt  : 기간 (str) (default="1")
        ...
        # result에 요청 종목 코드(code)가 따로 없기 때문에
        # result의 개별 딕셔너리에 code값을 추가 (추후 DB에 저장하기 위함)
        for item in result:
            item["code"] = code
            # 위 문장은 아래 문장들 중 하나로 대체 가능하다
            # item.update({"code":code})
            # item.update(dict(code=code))
            # item.update(code=code)
            # dict(item, **{"code":code})
            # [ dict(item, **{"code":code}) for item in result]
    ...
    def get_code_list(self, market=None):
        """
        TR : t8436
        코스피, 코스닥의 종목 리스트를 가져온다.

        [Parameters]
        market : 조회할 시장을 구분하는 코드로, 전체는 "ALL", 코스피는 "KOSPI", 코스닥은 "KOSDAQ"으로 구분 (str) (default=None)

        [Returns]
        result : 시장별 종목 리스트 (list)
        """
        ...
        result = self._execute_query("t8436", "t8436InBlock", "t8436OutBlock", *out_params, **in_params) # 종목조회(t8436) TR 요청
        return result
    ...
    def get_credit_trend_by_code(self, code=None, date=None):
        ...
        [Parameters]
        code : 종목코드 (str) (default=None)
        date : 조회할 날짜 8자리 (yyyymmdd) (str) (default=None)
    ...
    def get_agent_trend_by_code(self, code=None ,fromdt=None, todt=None):
        ...
        [Parameters]
        code   : 종목코드 (str) (default=None)
        fromdt : 조회 시작 날짜 8자리 (yyyymmdd) (str) (default=None)
        todt   : 조회 종료 날짜 8자리 (yyyymmdd) (str) (default=None)
    ...
    def get_short_trend_by_code(self, code=None, sdate=None, edate=None):
        ...
        [Parameters]
        code  : 종목코드 (str) (default=None)
        sdate : 조회 시작 날짜 8자리 (yyyymmdd) (str) (default=None)
        edate : 조회 종료 날짜 8자리 (yyyymmdd) (str) (default=None)
    ...
    def get_account_info(self):
        """
        TR: CSPAQ12200
        현물계좌 예수금/주문가능금액/총평가를 조회한다.

        [Parameters]
        No Parameters

        [Returns]
        result : 조회 결과 리스트 (list)
        """
    ...
    def order_check(self, order_no=None):
        ...
        [Parameters]
        order-no : 주문번호 (str) (default=None)

        [Returns]
        result : 해당 주문번호의 체결상태조회 결과 리스트 (dict)
        """
    ...
    def order_stock(self, code, qty, price, bns_type, order_type="00"):
        ...
        qty        : 주문수량 (str)
        price      : 가격 (int)
        bns_type   : 매매타입 ('1':매도, '2':매수) (str)
        order_type : 호가유형 ("00":지정가, "03":시장가, "05":조건부지정가, "07":최우선지정가, "61":장개시전시간외 종가, "81":시간외종가, "82":시간외단일가)(str)

        [Returns]
        result : 주문 관련정보 리스트 (dict)
        """
    ...
    def get_price_n_min_by_code(self, date, code, tick=None):
        ...
        date : 시작시간 (str)
        code : 종목코드 (str)
        tick : 호가단위 (str) (default=None)

        [Returns]
        result : 일일 분당 가격 정보 (dict)
        """
